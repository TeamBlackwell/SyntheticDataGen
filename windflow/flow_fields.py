    ...
    *,
    ...
    if not speed_x and not speed_y:
        if not speed_candidate_list:
            raise ValueError(
                "Either speed_x and speed_y must be provided or speed_candidate_list must be provided"
            )
        else:
            list_mode = True
    elif not speed_x or not speed_y:
        raise ValueError(
            "Both speed_x and speed_y must be provided if one is provided, else provide speed_candidate_list"
        )
    else:
        # both are provided
        list_mode = False
    ...
    gen_data_df = pd.DataFrame(
        columns=["cityscape", "speed_x", "speed_y", "pre_time", "post_time", "out_file"]
    )
    for cityscape_file in tqdm(cityscape_files, desc="Cityscape"):
        ...
        speed_x, speed_y = speed_candidate_list[
        ...
        gen_data_df.loc[len(gen_data_df)] = [
            str(cityscape_file),
            speed_x,
            speed_y,
            pre_time,
